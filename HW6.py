...
import random
listofproverbs = []
inputnumber = int(input("Введите число пословиц, которое хотите получить, но не более 55: "))
count = 0
if inputnumber > 55:
    print("Введенное количество превышает допустимое значение")
else:
    while count < inputnumber:
        randomnumber = random.randint(0, len(variants)-1)
        newvariant = variants.pop(randomnumber).capitalize()
        randomnumber2 = random.randint(0, len(proverbs)-1)
        newproverb = proverbs.pop(randomnumber2)
        newproverb2 = newproverb.replace("Ум", newvariant).replace("Умом", newvariant)
        listofproverbs.append(newproverb2)
        count = count + 1
print(listofproverbs)